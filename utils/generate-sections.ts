...
export const sectionToLiquid = (section, key) => {
  const shopifyThemeString = fs.readFileSync(
    path.join(process.cwd(), ".admirable-cms", "theme", "layout", "theme.liquid"),
    {
      encoding: "utf-8",
    }
  );
...
export const generateSections = async (
  ...
  for (const key in sections) {
    ...
    if (
      !fs.existsSync(path.join(process.cwd(), ".admirable-cms", "theme", "snippets", snippetName))
    ) {
      fs.writeFileSync(
        path.join(process.cwd(), ".admirable-cms", "theme", "snippets", snippetName),
        `<div></div>`
      );
      await api
        .put<Asset.Update>({
          path: `themes/${ADMIRABLE_CMS_THEME_ID}/assets`,
          type: DataType.JSON,
          data: {
            asset: {
    ...
    if (
      !fs.existsSync(path.join(process.cwd(), ".admirable-cms", "theme", "sections", sectionName))
    ) {
      fs.writeFileSync(
        path.join(process.cwd(), ".admirable-cms", "theme", "sections", sectionName),
        content
      );
      await api
        .put<Asset.Update>({
