...
import { MetafieldDefinitionsQuery, MetafieldDefinitionsQueryVariables } from "./shopify-graphql-types";
...
const getType = (type) => {
  ...
  export async function createMetafieldTypes(gql: GraphqlClient) {
    ...
    if (!fs.existsSync(path.join(process.cwd(), ".admirable-cms", "types", "metafields.ts"))) {
      console.log(chalk.green("created metafields.ts"));
      fs.writeFileSync(
        path.join(process.cwd(), ".admirable-cms", "types", "metafields.ts"),
        masterFile
      );
    }
