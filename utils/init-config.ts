...
export const initConfig = async (force: boolean) => {
  console.log({ fsExists: fs.existsSync(path.join(process.cwd(), ".admirable-cms")) });
  ...
  const config: Config = JSON.parse(
    fs.readFileSync(path.join(process.cwd(), ".admirable-cms", "config.json"), {
      encoding: "utf-8",
    })
  );
  ...
  if (
    ...
    if (fs.existsSync(path.join(process.cwd(), ".admirable-cms", "theme", "sections"))) {
      const sections = fs.readdirSync(
        path.join(process.cwd(), ".admirable-cms", "theme", "sections")
      );
      ...
      fs.writeFileSync(
        path.join(process.cwd(), ".admirable-cms", "config.json"),
