...
export const initBackup = async (
  ...
  try {
    ...
    if (!fs.existsSync(path.join(process.cwd(), ".admirable-cms", folder))) {
      fs.mkdirSync(path.join(process.cwd(), ".admirable-cms", folder));
    }
    if (!fs.existsSync(path.join(process.cwd(), ".admirable-cms", folder, "templates"))) {
      fs.mkdirSync(path.join(process.cwd(), ".admirable-cms", folder, "templates"));
    }
    if (!fs.existsSync(path.join(process.cwd(), ".admirable-cms", folder, "config"))) {
      fs.mkdirSync(path.join(process.cwd(), ".admirable-cms", folder, "config"));
    }
    files.map((file) => {
      fs.writeFileSync(
        path.join(process.cwd(), ".admirable-cms", folder, file.body.asset.key),
