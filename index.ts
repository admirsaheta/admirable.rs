...
const { ADMIRABLE_CMS_FOLDER } = process.env;
...
export const init = async () => {
  if (program.opts().types) {
    ...
    if (!fs.existsSync(path.join(process.cwd(), ".admirable-cms", "types", "settings.ts"))) {
      fs.writeFileSync(
        path.join(process.cwd(), ".admirable-cms", "types", "settings.ts"),
        `export type SettingsSchema = {};`
      );
      return;
    }

    if (!fs.existsSync(path.join(process.cwd(), ".admirable-cms", "types", "sections.ts"))) {
      fs.writeFileSync(
        path.join(process.cwd(), ".admirable-cms", "types", "sections.ts"),
    ...
    if (ADMIRABLE_CMS_THEME_ID) {
      await getSettings(api, ADMIRABLE_CMS_THEME_ID);
      await getSections(api, ADMIRABLE_CMS_THEME_ID);
    }
  ...
  const ADMIRABLE_CMS_THEME_ID = await initTheme(api, config);
  console.log(
  ...
  if (program.opts().update) {
    await updateTheme(api, ADMIRABLE_CMS_THEME_ID, config);
  }

  if (program.opts().backup) {
    await initBackup(api, ADMIRABLE_CMS_THEME_ID);
  }
  if (program.opts().download) {
    await initBackup(api, ADMIRABLE_CMS_THEME_ID, "theme");
  }

  if (!ADMIRABLE_CMS_FOLDER) return;

  console.log(
    `[${chalk.gray(new Date().toLocaleTimeString())}]: ${chalk.magentaBright(
      `Watching for changes in /${ADMIRABLE_CMS_FOLDER}/`
    )}`
  );

  if (fs.existsSync(path.join(process.cwd(), ADMIRABLE_CMS_FOLDER))) {
    watch(path.join(process.cwd(), ADMIRABLE_CMS_FOLDER), { recursive: true }, async (evt, name) => {
      if (!name.match(/\.(ts|tsx)$/)) return;
      if (name.match(/^index\.ts.$/)) return;
      if (name.match(/^_/)) return;

      const files = fs.readdirSync(path.join(process.cwd(), ADMIRABLE_CMS_FOLDER));
      ...
      const sections: { [T: string]: ShopifySection } = files
        .filter((name) => {
          ...
          const isDirectory = fs
            .statSync(path.join(process.cwd(), ADMIRABLE_CMS_FOLDER, name))
            .isDirectory();
        ...
        .reduce(
          (acc, file) => {
            try {
              const filename = path.join(process.cwd(), ADMIRABLE_CMS_FOLDER, file);
      ...
      await generateSections(api, ADMIRABLE_CMS_THEME_ID, sections);

      const settingsFilename = files.find((name) => name.match("settings_schema"));

      if (settingsFilename) {
        const filename = path.join(process.cwd(), ADMIRABLE_CMS_FOLDER, settingsFilename);
